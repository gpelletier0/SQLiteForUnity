...
public class AutoSavePreferences
{
	...
	static int SAVEINTERVALINDEX = 5;
    ...
    private class AutoSaveSettingsProvider : SettingsProvider
    {
        public AutoSaveSettingsProvider(string path, SettingsScope scope = SettingsScope.User) : base(path, scope) { }

        public override void OnGUI(string searchContext) => OldOnGUI();
    }

    [SettingsProvider]
    static SettingsProvider AutoSavePref() => new AutoSaveSettingsProvider("Preferences/AutoSave");
    
    public static void OldOnGUI()
	{
		if (!loaded) {
			LoadPreferences ();
		}
	...
	public static void LoadPreferences ()
	{
        ...
        if (saveIntervalIndex >= 0 && intervalValues.Length > saveIntervalIndex)
            saveInterval = intervalValues[saveIntervalIndex];
